...
contract BunnyFactory is IGlideFactory {
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(abi.encodePacked(type(BunnyPair).creationCode));
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'Bunny: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(BunnyPair).creationCode;
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, 'Bunny: FORBIDDEN');
        feeTo = _feeTo;
    ...
    function sortTokens(address tokenA, address tokenB) public pure override returns (address token0, address token1) {
        require(tokenA != tokenB, 'BunnyFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'BunnyFactory: ZERO_ADDRESS');
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure override returns (uint amountB) {
        require(amountA > 0, 'BunnyFactory: INSUFFICIENT_AMOUNT');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public view override returns (uint amountOut) {
        require(amountIn > 0, 'BunnyFactory: INSUFFICIENT_INPUT_AMOUNT');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public view override returns (uint amountIn) {
        require(amountOut > 0, 'BunnyFactory: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BunnyFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view override returns (uint[] memory amounts) {
        require(path.length >= 2, 'BunnyFactory: INVALID_PATH');
